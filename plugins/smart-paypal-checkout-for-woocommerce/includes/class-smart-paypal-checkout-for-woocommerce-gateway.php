...
class Paypal_Checkout_For_Woocommerce_Gateway extends WC_Payment_Gateway_CC {
    ...
    public function generate_psb_paypal_checkout_onboarding_html($field_key, $data) {
        if (isset($data['type']) && $data['type'] === 'psb_paypal_checkout_onboarding') {
            ...
            $label = ($testmode === 'no') ? __('Connect to PayPal', 'smart-paypal-checkout-for-woocommerce') : __('Connect to PayPal Sandbox', 'smart-paypal-checkout-for-woocommerce');
            ob_start();
            ?>
            <tr valign="top">
                <th scope="row" class="titledesc">
    ...
    public function psb_admin_notice() {
        ...
        if (false !== get_transient('psb_primary_email_not_confirmed')) {
            delete_transient('psb_primary_email_not_confirmed');
        ...
        $message = sprintf(
                __(
                        'PayPal Complete Payments is almost ready. To get started, <a href="%1$s">connect your account</a>.', 'smart-paypal-checkout-for-woocommerce'

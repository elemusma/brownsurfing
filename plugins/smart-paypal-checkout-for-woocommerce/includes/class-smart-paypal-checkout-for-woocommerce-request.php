...
class Paypal_Checkout_For_Woocommerce_Request extends WC_Payment_Gateway {
    ...
    public function __construct() {
        try {
            ...
            if ($this->is_sandbox) {
                ...
                $this->webhook_id = 'psb_sandbox_webhook_id';
                ...
                $this->basicAuth = base64_encode($this->client_id . ":" . $this->secret);
                $this->webhook_id = 'psb_live_webhook_id';
            ...
            $this->decimals = $this->psb_get_number_of_decimal_digits();
    ...
    public function get_genrate_token() {
        try {
            if ($this->is_valid_for_use() === true && $this->access_token) {
                ...
                $this->psb_log('Get Genrate token Request' . $this->paypal_oauth_api);
                if (is_wp_error($response)) {
                    $error_message = $response->get_error_message();
                    $this->psb_log('Error Message : ' . wc_print_r($error_message, true));
                } else {
                    $api_response = json_decode(wp_remote_retrieve_body($response), true);
                    $this->psb_log('Response Code: ' . wp_remote_retrieve_response_code($response));
                    $this->psb_log('Response Message: ' . wp_remote_retrieve_response_message($response));
                    $this->psb_log('Response Body: ' . wc_print_r($api_response, true));
    ...
    public function psb_log($message, $level = 'info') {
        if ($this->log_enabled) {
        ...
        try {
                ...
                array(
                    ...
                    array(
                        ...
                        array(
                            ...
                            'value' => psb_ppcp_round($cart['order_total'], $this->decimals),
                            'breakdown' => array()
                        )
                    ),
                ),
            ...
            if (isset($cart['total_item_amount']) && $cart['total_item_amount'] > 0) {
                $body_request['purchase_units'][0]['amount']['breakdown']['item_total'] = array(
                    'currency_code' => get_woocommerce_currency(),
                    'value' => psb_ppcp_round($cart['total_item_amount'], $this->decimals)
                );
            }
            if (isset($cart['shipping']) && $cart['shipping'] > 0) {
                $body_request['purchase_units'][0]['amount']['breakdown']['shipping'] = array(
                    'currency_code' => get_woocommerce_currency(),
                    'value' => psb_ppcp_round($cart['shipping'], $this->decimals)
                );
            }
            ...
            if (isset($cart['items']) && !empty($cart['items'])) {
                foreach ($cart['items'] as $key => $order_items) {
                    ...
                    $body_request['purchase_units'][0]['items'][$key] = array(
                        ...
                        array(
                            'currency_code' => get_woocommerce_currency(),
                            'value' => psb_ppcp_round($order_items['amount'], $this->decimals)
                        ),
                    );
                }
            }
            ...
            $this->psb_log('Order Request : ' . wc_print_r($body_request, true));
            ...
            if (is_wp_error($response)) {
                ...
                if (!empty($api_response['status'])) {
                    ...
                    $this->psb_log('Error Message : ' . wc_print_r($api_response, true));
    ...
    public function psb_get_extra_offset_line_item($amount) {
        try {
            return array(
                ...
                'quantity' => 1,
                'amount' => psb_ppcp_round($amount, $this->decimals),
            );
        } catch (Exception $ex) {
    ...
    public function psb_get_access_token() {
        try {
            ...
            $this->psb_log('Get access token Request' . $this->paypal_oauth_api);
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $this->psb_log('Error Message : ' . wc_print_r($error_message, true));
            } else {
    ...
    public function psb_order_capture_request($woo_order_id, $need_to_update_order = true) {
        try {
            ...
            $this->psb_log('Request : ' . wc_print_r($this->paypal_order_api . $paypal_order_id . '/capture', true));
            ...
            if (is_wp_error($response)) {
                ...
                $return_response = array();
                $api_response = json_decode(wp_remote_retrieve_body($response), true);
                $this->psb_log('Response : ' . wc_print_r($api_response, true));
                if (isset($api_response['id']) && !empty($api_response['id'])) {
                    ...
                    if ($api_response['status'] == 'COMPLETED') {
                        ...
                        if ($payment_status == 'COMPLETED') {
                            ...
                            psb_update_woo_order_status($woo_order_id, $payment_status, $payment_status_reason);
                        }
    ...
    public function psb_order_auth_request($woo_order_id) {
        try {
            ...
            $this->psb_log('Request : ' . wc_print_r($this->paypal_order_api . $paypal_order_id . '/authorize', true));
    ...
    public function psb_get_checkout_details($paypal_order_id) {
        try {
            ...
            $this->psb_log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id);
    ...
    public function psb_get_details_from_cart() {
        try {
            ...
            $details = array(
                'total_item_amount' => psb_ppcp_round(WC()->cart->cart_contents_total, $this->decimals) + $discounts,
                'order_tax' => psb_ppcp_round(WC()->cart->tax_total + WC()->cart->shipping_tax_total, $this->decimals),
                'shipping' => psb_ppcp_round(WC()->cart->shipping_total, $this->decimals),
    ...
    public function psb_get_rounded_total_in_cart() {
        try {
            ...
            foreach (WC()->cart->cart_contents as $cart_item_key => $values) {
                $amount = psb_ppcp_round($values['line_subtotal'] / $values['quantity'], $this->decimals);
                $rounded_total += psb_ppcp_round($amount * $values['quantity'], $this->decimals);
            }
    ...
    public function psb_get_details($details, $discounts, $rounded_total, $total) {
        try {
            $discounts = psb_ppcp_round($discounts, $this->decimals);
            $details['order_total'] = psb_ppcp_round(
            ...
            if ($details['total_item_amount'] != $rounded_total) {
                ...
                if (abs($diff) > 0.000001 && 0.0 !== (float) $diff) {
                    $extra_line_item = $this->psb_get_extra_offset_line_item($diff);
                    $details['items'][] = $extra_line_item;
                    $details['total_item_amount'] += $extra_line_item['amount'];
                    $details['total_item_amount'] = psb_ppcp_round($details['total_item_amount'], $this->decimals);
                    $details['order_total'] += $extra_line_item['amount'];
                    $details['order_total'] = psb_ppcp_round($details['order_total'], $this->decimals);
                }
            }
            ...
            $wc_order_total = psb_ppcp_round($total, $this->decimals);
            $discounted_total = psb_ppcp_round($details['order_total'], $this->decimals);
            if ($wc_order_total != $discounted_total) {
                if ($discounted_total < $wc_order_total) {
                    $details['order_tax'] += $wc_order_total - $discounted_total;
                    $details['order_tax'] = psb_ppcp_round($details['order_tax'], $this->decimals);
                } else {
                    $details['ship_discount_amount'] += $wc_order_total - $discounted_total;
                    $details['ship_discount_amount'] = psb_ppcp_round($details['ship_discount_amount'], $this->decimals);
    ...
    public function psb_get_details_from_order($order_id) {
        try {
            ...
            $details = array(
                'total_item_amount' => psb_ppcp_round($order->get_subtotal(), $this->decimals),
                'order_tax' => psb_ppcp_round($order->get_total_tax(), $this->decimals),
                'shipping' => psb_ppcp_round(( version_compare(WC_VERSION, '3.0', '<') ? $order->get_total_shipping() : $order->get_shipping_total()), $this->decimals),
    ...
    public function psb_get_paypal_line_items_from_order($order) {
        try {
            ...
            foreach ($order->get_items() as $cart_item_key => $values) {
                $desc = '';
                $amount = psb_ppcp_round($values['line_subtotal'] / $values['qty'], $this->decimals);
    ...
    public function psb_get_rounded_total_in_order($order) {
        try {
            ...
            foreach ($order->get_items() as $cart_item_key => $values) {
                $amount = psb_ppcp_round($values['line_subtotal'] / $values['qty'], $this->decimals);
                $rounded_total += psb_ppcp_round($amount * $values['qty'], $this->decimals);
            }
            return $rounded_total;
        } catch (Exception $ex) {
    ...
    public function psb_refund_order($order_id, $amount, $reason, $transaction_id) {
        try {
            ...
            $this->psb_log('Endpoint: ' . $this->paypal_refund_api . $transaction_id . '/refund');
            $order = wc_get_order($order_id);
            $this->decimals = $this->psb_get_number_of_decimal_digits();
            ...
            if (!empty($amount) && $amount > 0) {
                $body_request = array(
                    'amount' =>
                    array(
                        'value' => psb_ppcp_round($amount, $this->decimals),
            ...
            $body_request = psb_remove_empty_key($body_request);
            $body_request = json_encode($body_request);
            $this->psb_log('Refund request: ' . $body_request);
            ...
            if (isset($api_response['status']) && $api_response['status'] == "COMPLETED") {
                ...
                $order->add_order_note(sprintf(__('Payment via %s Pending. Pending reason: %s.', 'smart-paypal-checkout-for-woocommerce'), $order->get_payment_method_title(), $pending_reason_text));
                $order->add_order_note(
                        sprintf(__('Refund Amount %1$s - Refund ID: %2$s', 'smart-paypal-checkout-for-woocommerce'), $gross_amount, $refund_transaction_id)
                );
            } else {
    ...
    public function psb_update_order($order) {
        try {
            ...
            $this->psb_log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id);
            $this->psb_log('Request: ' . print_r($patch_request_json, true));
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $this->psb_log('Error Message : ' . wc_print_r($response, true));
                wc_add_notice($error_message, 'error');
                return false;
            } else {
                $api_response = json_decode(wp_remote_retrieve_body($response), true);
    ...
    public function psb_show_details_authorized_payment($authorization_id) {
        try {
            ...
            $this->psb_log('Endpoint: ' . $this->auth . $authorization_id);
    ...
    public function psb_capture_authorized_payment($woo_order_id) {
        try {
            ...
            $capture_arg = array(
                'amount' =>
                array(
                    'value' => psb_ppcp_round($order->get_total(), $this->decimals),
            ...
            $this->psb_log('Request : ' . wc_print_r($this->auth . $authorization_id . '/capture', true));
    ...
    public function psb_add_log_details($action_name = null) {
        $this->psb_log(sprintf(__('Smart PayPal Checkout For WooCommerce Version: %s', 'smart-paypal-checkout-for-woocommerce'), SMART_PAYPAL_CHECKOUT_FOR_WOOCOMMERCE_VERSION));
        $this->psb_log(sprintf(__('WooCommerce Version: %s', 'smart-paypal-checkout-for-woocommerce'), WC_VERSION));
        $mode = $this->is_sandbox ? 'Yes' : 'No';
        $this->psb_log("Test Mode: " . $mode);
        $this->psb_log('Action Name : ' . $action_name);
    }
        ...
        try {
            if ($this->is_valid_for_use() === true && $this->access_token) {
                ...
                if (is_wp_error($response)) {
                    ...
                    $return_response = array();
                    $api_response = json_decode(wp_remote_retrieve_body($response), true);
                    $this->psb_log('function called: psb_create_webhooks_request');
                    if (!empty($api_response['id'])) {
                        $this->psb_log('Response Code: ' . wp_remote_retrieve_response_code($response));
                        $this->psb_log('Response Message: ' . wp_remote_retrieve_response_message($response));
                        $this->psb_log('Response Body: ' . wc_print_r($api_response, true));
                        ...
                        $this->psb_log('Response Message: ' . wc_print_r($error, true));
    ...
    public function psb_handle_webhook_request_handler() {
        try {
            ...
            if ($this->is_valid_for_use() === true && $this->access_token) {
                ...
                $this->psb_log('Response Body: ' . wc_print_r($posted, true));
                $this->psb_log('Headers: ' . wc_print_r($headers, true));
    ...
    public function psb_validate_webhook_event($headers, $body) {
        try {
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $this->psb_log('Webhook Error Message : ' . wc_print_r($error_message, true));
                return false;
            } else {
                $return_response = array();
                $api_response = json_decode(wp_remote_retrieve_body($response), true);
    ...
    public function psb_update_order_status($posted) {
        ...
        if ($order && isset($posted['event_type']) && !empty($posted['event_type'])) {
            ...
            if (isset($posted['resource']['status']) && !empty($posted['resource']['status'])) {
                $this->psb_log('Payment status: ' . $posted['resource']['status']);
            }
            if (isset($posted['resource']['id']) && !empty($posted['resource']['id'])) {
                $this->psb_log('PayPal Transaction ID: ' . $posted['resource']['id']);
            }
    ...
    public function payment_status_completed($order, $posted) {
        if ($order->has_status(wc_get_is_paid_statuses())) {
            $this->psb_log('Aborting, Order #' . $order->get_id() . ' is already complete.');
            exit;
        }
    ...
    public function psb_get_paypal_order($raw_custom) {
        ...
        if ($custom && is_object($custom)) {
            ...
            $this->psb_log('Order ID and key were not found in "custom_id".');
            return false;
        }
        $order = wc_get_order($order_id);
        if (!$order) {
        ...
        if (!$order || !hash_equals($order->get_order_key(), $order_key)) {
            $this->psb_log('Order Keys do not match.');
            return false;
        }
        $this->psb_log('Order  match : ' . $order_id);

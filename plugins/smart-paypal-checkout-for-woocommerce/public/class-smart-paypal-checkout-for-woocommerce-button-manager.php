...
class Paypal_Checkout_For_Woocommerce_Button_Manager {
    ...
    public function get_properties() {
        ...
        $this->enable_product_button = 'yes' === $this->psb_get_settings('enable_product_button', 'yes');
        $this->enable_cart_button = 'yes' === $this->psb_get_settings('enable_cart_button', 'yes');
        $this->enable_checkout_button = 'yes' === $this->psb_get_settings('enable_checkout_button', 'yes');
        $this->enable_mini_cart_button = 'yes' === $this->psb_get_settings('enable_mini_cart_button', 'yes');
        ...
        $this->threed_secure_contingency = $this->psb_get_settings('3d_secure_contingency', 'SCA_WHEN_REQUIRED');
    ...
    public function psb_add_hooks() {
        ...
        add_action('wp_loaded', array($this, 'psb_create_webhooks'));
        add_action('woocommerce_pay_order_after_submit', array($this, 'psb_add_order_id'));
        add_action('wp_loaded', array($this, 'psb_prevent_add_to_cart_woo_action'), 1);
    }
    ...
    public function enqueue_scripts() {
        ...
        $this->psb_smart_button_style_properties();
        $smart_js_arg = array();
        ...
        wp_localize_script($this->plugin_name, 'psb_manager', array(
            ...
            'create_order_url' => add_query_arg(array('psb_action' => 'create_order', 'utm_nooverride' => '1', 'from' => is_checkout_pay_page() ? 'pay_page' : $page), WC()->api_request_url('Paypal_Checkout_For_Woocommerce_Button_Manager')),
    ...
    public function display_paypal_button_cart_page() {
        ...
        echo '<div class="psb-button-container"><div class="psb_cart"></div><div class="psb-proceed-to-checkout-button-separator">&mdash; ' . __('OR', 'smart-paypal-checkout-for-woocommerce') . ' &mdash;</div></div>';
    }
    ...
    public function display_paypal_button_product_page() {
        ...
        wp_enqueue_script('psb-checkout-js');
        wp_enqueue_script($this->plugin_name);
        echo '<div class="psb-button-container"><div class="psb_product"></div></div>';
    }
    ...
    public function display_paypal_button_checkout_page() {
        if (has_active_session() === false) {
            wp_enqueue_script('psb-checkout-js');
            wp_enqueue_script($this->plugin_name);
            echo '<div class="psb-button-container"><div class="psb_checkout"></div><div class="psb-proceed-to-checkout-button-separator checkout_cc_separator">&mdash;&mdash; ' . __('OR', 'smart-paypal-checkout-for-woocommerce') . ' &mdash;&mdash;</div></div>';
        }
    }
    ...
    public function paypal_shipping_details() {
        ...
        public function get_mapped_billing_address($checkout_details) {
            ...
            $phone = '';
            if (!empty($this->checkout_details->payer->phone->phone_number->national_number)) {
        ...
        public function psb_display_order_page() {
            ...
            if ($this->paymentaction === 'capture' && !empty($this->checkout_details->status) && $this->checkout_details->status == 'COMPLETED' && $order !== false) {
                ...
                if ($payment_status == 'COMPLETED') {
                    ...
                    psb_update_woo_order_status($order_id, $payment_status, $payment_status_reason);
                }
        ...
        public function psb_create_webhooks() {
            if (false === get_transient('psb_is_webhook_process_started')) {
                if (psb_is_local_server() === false) {
                    $webhook_id = get_option($this->webhook_id, '');
                    if (empty($webhook_id)) {
                        $this->request->psb_create_webhooks_request();
                    }
                    set_transient('psb_is_webhook_process_started', 'done', 3 * HOUR_IN_SECONDS);
                }
            }
        }
        ...
        public function psb_cc_capture() {
            try {
                ...
                if (!empty($psb_paypal_order_id)) {
                    ...
                    $order = wc_get_order($order_id);
                    $liability_shift_result = $this->psb_liability_shift($order, $api_response);
                    if ($liability_shift_result === 1) {
                        ...
                        $is_success = false;
                        wc_add_notice(__('Something went wrong. Please try again.', 'smart-paypal-checkout-for-woocommerce'), 'error');
                    }
                    if ($is_success) {
                        WC()->cart->empty_cart();
                        unset(WC()->session->psb_session);
        ...
        public function psb_add_order_id() {
            global $wp;
        ...
        public function psb_smart_button_style_properties() {
            $this->disable_funding = array();
            $this->style_layout = $this->psb_get_settings('cart_button_layout', 'vertical');
            $this->style_color = 'gold';
            $this->style_shape = 'rect';
            $this->style_label = 'paypal';
            $this->style_tagline = 'yes';
            if (is_product()) {
                $this->disable_funding = $this->psb_get_settings('product_disallowed_funding_methods', array());
                $this->style_layout = $this->psb_get_settings('product_button_layout', 'horizontal');
                $this->style_color = $this->psb_get_settings('product_style_color', 'gold');
                $this->style_shape = $this->psb_get_settings('product_style_shape', 'rect');
                $this->style_label = $this->psb_get_settings('product_button_label', 'paypal');
                $this->style_tagline = $this->psb_get_settings('product_button_tagline', 'yes');
            } elseif (is_cart()) {
                $this->disable_funding = $this->psb_get_settings('cart_disallowed_funding_methods', array());
                $this->style_layout = $this->psb_get_settings('cart_button_layout', 'vertical');
                $this->style_color = $this->psb_get_settings('cart_style_color', 'gold');
                $this->style_shape = $this->psb_get_settings('cart_style_shape', 'rect');
                $this->style_label = $this->psb_get_settings('cart_button_label', 'paypal');
                $this->style_tagline = $this->psb_get_settings('cart_button_tagline', 'yes');
            } elseif (is_checkout() || is_checkout_pay_page()) {
                $this->disable_funding = $this->psb_get_settings('checkout_disallowed_funding_methods', array());
                $this->style_layout = $this->psb_get_settings('checkout_button_layout', 'vertical');
                $this->style_color = $this->psb_get_settings('checkout_style_color', 'gold');
                $this->style_shape = $this->psb_get_settings('checkout_style_shape', 'rect');
                $this->style_label = $this->psb_get_settings('checkout_button_label', 'paypal');
                $this->style_tagline = $this->psb_get_settings('checkout_button_tagline', 'yes');
            }
            $this->mini_cart_disable_funding = $this->psb_get_settings('mini_cart_disallowed_funding_methods', array());
            $this->mini_cart_style_layout = $this->psb_get_settings('mini_cart_button_layout', 'vertical');
            $this->mini_cart_style_color = $this->psb_get_settings('mini_cart_style_color', 'gold');
            $this->mini_cart_style_shape = $this->psb_get_settings('mini_cart_style_shape', 'rect');
            $this->mini_cart_style_label = $this->psb_get_settings('mini_cart_button_label', 'paypal');
            $this->mini_cart_style_tagline = $this->psb_get_settings('mini_cart_button_tagline', 'yes');
        }

        public function psb_prevent_add_to_cart_woo_action() {
            if (isset($_REQUEST['psb-add-to-cart'])) {
